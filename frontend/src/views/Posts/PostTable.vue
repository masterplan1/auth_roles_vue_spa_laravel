    ...
    <table class="table-auto w-full">
      ...
      <tbody>
        <tr v-for="post in posts" :key="post.id">
          <td class="border-b border-gray-400 p-2">{{ post.id }}</td>
          <td class="border-b border-gray-400 p-2">{{ post.title }}</td>
          <td class="border-b border-gray-400 p-2">{{ post.created_at }}</td>
          <td class="border-b border-gray-400 p-2">
            <div class="flex gap-2">
              ...
              >
                <router-link
                  :to="{ name: 'app.post-edit', params: { id: post.id } }"
                >
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    fill="none"
...
function removePost(id) {
  axiosClient.delete("/posts/" + id).then(() => {
    store.getPosts();
  });
}

onMounted(() => {
  store.getPosts().then((res) => {});
});
</script>